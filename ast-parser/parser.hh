...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // expression
      char dummy2[sizeof (BaseExpression*)];

      // base_statement
      char dummy3[sizeof (BaseStatement*)];

      // call_to_print
      char dummy4[sizeof (CallToPrint*)];

      // cond_clause
      char dummy5[sizeof (CondClause*)];

      // conditional
      char dummy6[sizeof (Conditional*)];

      // declaration
      char dummy7[sizeof (Declaration*)];
      ...
      // statements
      char dummy10[sizeof (Statements*)];
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_expression: // expression
        value.move< BaseExpression* > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_call_to_print: // call_to_print
        value.move< CallToPrint* > (std::move (that.value));
        break;

      case symbol_kind::S_cond_clause: // cond_clause
        value.move< CondClause* > (std::move (that.value));
        break;

      case symbol_kind::S_conditional: // conditional
        value.move< Conditional* > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, BaseExpression*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const BaseExpression*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, CallToPrint*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const CallToPrint*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, CondClause*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const CondClause*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Conditional*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Conditional*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_expression: // expression
        value.template destroy< BaseExpression* > ();
        break;
      ...
      case symbol_kind::S_call_to_print: // call_to_print
        value.template destroy< CallToPrint* > ();
        break;

      case symbol_kind::S_cond_clause: // cond_clause
        value.template destroy< CondClause* > ();
        break;

      case symbol_kind::S_conditional: // conditional
        value.template destroy< Conditional* > ();
        break;
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_FUNC_MAIN (location_type l)
      {
        return symbol_type (token::TOK_FUNC_MAIN, std::move (l));
      }
#else
      static
      symbol_type
      make_FUNC_MAIN (const location_type& l)
      {
        return symbol_type (token::TOK_FUNC_MAIN, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_expression: // expression
        value.copy< BaseExpression* > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_call_to_print: // call_to_print
        value.copy< CallToPrint* > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_cond_clause: // cond_clause
        value.copy< CondClause* > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_conditional: // conditional
        value.copy< Conditional* > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_expression: // expression
        value.move< BaseExpression* > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_call_to_print: // call_to_print
        value.move< CallToPrint* > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_cond_clause: // cond_clause
        value.move< CondClause* > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_conditional: // conditional
        value.move< Conditional* > (YY_MOVE (s.value));
        break;
